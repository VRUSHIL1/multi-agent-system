...
class SessionsService:
    ...
    async def create_session_service(
        ...
        try:
            session = ChatSession(title=payload.title or "New Session", user_id=user.id)
            self.db.add(session)
    ...
    async def list_sessions_service(self, user: CurrentUser) -> list[ChatSession]:
    ...
    async def get_session_service(self, user_id: int, session_id: int) -> dict | None:
        stmt = select(ChatSession.id, ChatSession.title, ChatSession.user_id).where(
    ...
    async def delete_session_service(self, user_id: int, session_id: int) -> bool:
