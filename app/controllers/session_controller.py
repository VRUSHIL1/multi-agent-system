...
class SessionController:
    ...
    async def create_session(
        self, payload: SessionCreate, user: CurrentUser
    ) -> JSONResponse:
        session = await self.sessions_service.create_session_service(payload, user)
        if not session:
            raise ErrorResponse(500, "Failed to create session")
    ...
    async def get_session(self, session_id: int, user: CurrentUser) -> JSONResponse:
        ...
        return success_response(
            data=session, message="Session retrieved successfully", status_code=200
        )
    ...
    async def get_all_sessions(
        ...
        result = await self.sessions_service.get_all_sessions(
    ...
    async def get_session_history(
        self, user_id: int, session_id: int, db: AsyncSession
    ) -> JSONResponse:
        session = await self.sessions_service.get_session_service(user_id, session_id)
        if not session:
            raise ErrorResponse(404, "Session not found")
    ...
    async def delete_session(self, session_id: int, user: CurrentUser) -> JSONResponse:
        deleted = await self.sessions_service.delete_session_service(
            user.id, session_id
        )
        if not deleted:
            raise ErrorResponse(404, "Session not found")
